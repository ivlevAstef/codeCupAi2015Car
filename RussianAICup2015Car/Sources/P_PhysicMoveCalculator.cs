  ...
  public class MovingCalculator {
    ...
    private double wheelTurnForCurrentState(double angle) {
      if (null != needPos) {
        ...
        return new PCar(car, game).WheelTurnForEndZeroWheelTurn(car.GetAbsoluteAngleTo(defaultPos.X, defaultPos.Y), speedSign);
      }
    }
    ...
    private void turn(Move moveResult, Vector needDirAngle) {
      ...
      for (int ticksCount = 0; ticksCount < maxIterationCount;) {
        IPhysicEvent mapCrash = calculateTurnMapCrashEvents(iterCar, needDirAngle, false);
        ...
        if (null == mapCrash) {
          ...
          if (0 == ticksCount && !hasReserveTicks(iterCar, needDirAngle)) {
            ...
            if (!moveResult.IsBrake || null == mapBrakeCrash) {
        ...
        ticksCount += moveOnDistance(iterCar, needDirAngle, (endPoint - crashInfo.Item1).Length);
      }
    }
    ...
    private void avoidSideCrash(Move moveResult, Vector needDirAngle = null) {
      ...
      IPhysicEvent objectCrash = events.ComeContaints(PhysicEventType.ObjectsCrash) ? events.GetEvent(PhysicEventType.ObjectsCrash) : null;

      IPhysicEvent crash = (null != objectCrash) ? objectCrash : mapCrash;
    ...
    private void moveToAdditionalPoint(double needAngle) {
      ...
      foreach (Tuple<Vector, double> data in additionalPoints) {
        ...
        if (pEvents.ComeContaints(PhysicEventType.ObjectsCrash)) {
          continue;
        }*/
    ...
    private bool moveToAddPoint2EventCheckEnd(PCar physicCar, HashSet<IPhysicEvent> pEvents, int tick) {
      ...
      return pEvents.ComeContaints(PhysicEventType.ObjectsCrash) || pEvents.ComeContaints(PhysicEventType.SpeedReach);
    }

    /// Turn MapCrash
    private IPhysicEvent calculateTurnMapCrashEvents(PCar iterCar, Vector needDirAngle, bool isBrake) {
      HashSet<IPhysicEvent> pEvents = new HashSet<IPhysicEvent> {
